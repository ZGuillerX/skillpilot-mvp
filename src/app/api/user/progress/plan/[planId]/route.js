import { NextResponse } from 'next/server';
import pool from '@/lib/db';
import { verifyToken } from '@/lib/auth';

// DELETE: Eliminar un plan individual
export async function DELETE(request, { params }) {
    try {
        const token = request.headers.get('authorization')?.replace('Bearer ', '');

        if (!token) {
            return NextResponse.json(
                { error: 'Token no proporcionado' },
                { status: 401 }
            );
        }

        const decoded = await verifyToken(token);
        const resolvedParams = await params;
        const { planId } = resolvedParams;

        if (!planId) {
            return NextResponse.json(
                { error: 'Plan ID requerido' },
                { status: 400 }
            );
        }

        // Obtener progreso actual
        const [rows] = await pool.query(
            'SELECT learning_plan, challenge_history FROM user_progress WHERE user_id = ?',
            [decoded.userId]
        );

        if (rows.length === 0) {
            return NextResponse.json(
                { error: 'Progreso no encontrado' },
                { status: 404 }
            );
        }

        let learningPlanData = typeof rows[0].learning_plan === 'string'
            ? JSON.parse(rows[0].learning_plan)
            : rows[0].learning_plan;

        let history = rows[0].challenge_history
            ? (typeof rows[0].challenge_history === 'string'
                ? JSON.parse(rows[0].challenge_history)
                : rows[0].challenge_history)
            : [];

        // Filtrar el plan a eliminar
        const planIndex = learningPlanData.plans?.findIndex(p => p.id === planId);

        if (planIndex === -1) {
            return NextResponse.json(
                { error: 'Plan no encontrado' },
                { status: 404 }
            );
        }

        // Eliminar plan
        learningPlanData.plans.splice(planIndex, 1);

        // Si el plan eliminado era el activo, establecer el primero disponible
        if (learningPlanData.currentPlan?.id === planId) {
            learningPlanData.currentPlan = learningPlanData.plans[0] || null;
        }

        // Eliminar retos asociados a este plan
        history = history.filter(entry => entry.planId !== planId);

        // Actualizar en BD
        await pool.query(
            'UPDATE user_progress SET learning_plan = ?, challenge_history = ?, updated_at = NOW() WHERE user_id = ?',
            [JSON.stringify(learningPlanData), JSON.stringify(history), decoded.userId]
        );

        return NextResponse.json({
            success: true,
            message: 'Plan eliminado exitosamente',
            remainingPlans: learningPlanData.plans.length,
        });
    } catch (error) {
        console.error('Error deleting plan:', error);
        return NextResponse.json(
            { error: 'Error al eliminar el plan' },
            { status: 500 }
        );
    }
}
