...
function HistoryContent() {
  ...
  // Filtrar retos por plan
  const filteredHistory = planId
    ? history.filter((entry) => entry.planId === planId)
    : history.filter((entry) => entry.planId === currentPlan?.id);

  const completedChallenges = filteredHistory.filter(
    (entry) => entry.evaluation?.success
  );
  const attemptedChallenges = filteredHistory.filter(
    (entry) => entry.evaluation && !entry.evaluation.success
  );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            {currentPlan ? (
              <p className="text-muted-foreground">Plan: {currentPlan.goal}</p>
            ) : (
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              ...
              <span>•</span>
              <span>
                {entry.attempts || 1} intento{entry.attempts !== 1 ? "s" : ""}
              </span>
            </div>
          </div>
