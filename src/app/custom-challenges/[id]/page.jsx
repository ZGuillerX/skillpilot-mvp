...
export default function CustomChallengeEditorPage() {
  ...
  const handleSubmitCode = async () => {
    ...
    const hasContent = files.some(
      (f) => f.content && f.content.trim() && f.content.trim() !== (f.placeholder || "").trim()
    );
    ...
    try {
      ...
      console.error(err);
      setEvaluation({ success: false, score: 0, feedback: "Error al evaluar. Intenta de nuevo.", suggestions: [] });
    } finally {
  ...
  const retryChallenge = () => {
    setFiles((prev) => prev.map((f) => ({ ...f, content: f.placeholder || "// Tu código aquí\n" })));
    setEvaluation(null);
    setTimer(0);
    setTimerRunning(true);
    setChallengeStartTime(Date.now());
      ...
      <section className="flex-shrink-0 flex flex-col gap-2 px-6 py-2 border-b border-border bg-white/90 dark:bg-[#16161a]/90 shadow-sm">
        <div className="flex items-start justify-between gap-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-foreground mb-0.5">Reto Personalizado</h1>
              <p className="text-xs text-muted-foreground">{challenge?.language} • {challenge?.estimatedTimeMinutes} min estimado</p>
            </div>
          </div>
        </div>
